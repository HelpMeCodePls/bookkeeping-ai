...
import { PieChart, Pie, Cell, Tooltip, LineChart, Line, XAxis, YAxis, CartesianGrid, BarChart, Bar, ResponsiveContainer, Legend } from 'recharts';
...
export default function AnalysisPage() {
  ...
  // 获取分析数据
  const { data = { byCategory: {}, daily: [], minDate: '', maxDate: '' }, isLoading } = useQuery({
    queryKey: ['charts', ledgerId, mode, selectedDate],
    queryFn: () => axios.get('/charts/summary', { params: { ledgerId, mode, selectedDate } }).then(r => r.data),
    enabled: !!ledgerId,
  });
  ...
  const pieData = useMemo(() => Object.entries(data.byCategory).map(([name, value]) => ({ name, value })), [data]);
  const lineData = useMemo(() => data.daily.map(([date, value]) => ({ date, value })), [data]);
  ...
  const totalSpending = useMemo(() => {
    ...
    if (mode === 'all') {
      return Object.values(ledger.spent).reduce((sum, amount) => sum + amount, 0);
    }
    return ledger.spent[selectedDate] || 0;
  }, [ledger?.spent, mode, selectedDate]);

  const avgDaily = lineData.length ? (totalSpending / lineData.length).toFixed(2) : 0;
  ...
  function getOptionsByMode(mode) {
    const min = data.minDate ? dayjs(data.minDate) : dayjs().subtract(1, 'year');
    ...
    if (mode === 'week') {
      ...
      while (start.isBefore(max)) {
        ...
        options.push(`${start.format('YYYY-MM-DD')} ~ ${end.format('YYYY-MM-DD')}`);
    ...
    <div className="p-6 space-y-8">
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex gap-2">
          {['week', 'month', 'year', 'all'].map(m => (
            <button key={m} onClick={() => { setMode(m); setSelectedDate(''); }}
              className={`px-3 py-1 rounded-md ${mode === m ? 'bg-blue-600 text-white' : 'bg-gray-100 hover:bg-gray-200'}`}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-white shadow rounded-xl p-4 text-center">
          ...
          <p className="text-xl font-semibold mt-1">${totalSpending.toFixed(2)}</p>
        </div>
        <div className="bg-white shadow rounded-xl p-4 text-center">
      ...
      <div className="bg-white p-6 rounded-lg shadow">
        ...
        {isLoading ? <div className="h-60 flex justify-center items-center">Loading...</div> : (
      ...
      {/* 柱状图 */}
      <div className="bg-white p-6 rounded-lg shadow">
        <h3 className="text-blue-600 font-semibold mb-4">Spending by Category</h3>
        {isLoading ? <div className="h-72 flex items-center justify-center">Loading Chart...</div> : (
          <ResponsiveContainer width="100%" height={300}>
      ...
      {top5.length > 0 && (
        <div className="bg-white shadow p-4 rounded-lg">
          <h3 className="text-blue-600 font-semibold mb-4">Top 5 Categories</h3>
          ...
          <table className="min-w-full text-sm">
            <thead>
              <tr className="border-b">
                {['date', 'value'].map(col => (
                  <th key={col} className={`p-2 cursor-pointer ${col === 'value' ? 'text-right' : 'text-left'}`} onClick={() => sortBy(col)}>
                    {col.toUpperCase()} {sort.key === col && (sort.order === 'asc' ? '↑' : '↓')}
                  </th>
                ))}
              </tr>
            </thead>
            <tbody>
              {paginated.map((r, idx) => (
                <tr key={idx} className="border-b hover:bg-gray-50">
                  ...
                  <td className="p-2 text-blue-600 font-semibold text-right">${r.value.toFixed(2)}</td>
                </tr>
              ))}
            </tbody>
          </table>
