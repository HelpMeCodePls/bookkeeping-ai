...
const { data: ledger } = useQuery({
    ...
    queryFn: () => axios.get(`/ledgers/${currentId}`).then(r => ({
      ...r.data,
...
const checkBudget = async (month) => {
    ...
    onSuccess: (_, payload) => {
      ...
      queryClient.setQueryData(['ledgers', currentId], (oldData) => {
        ...
        // 更新分类预算
          ...
          updatedData.budgets.categoryBudgets = updatedData.budgets.categoryBudgets || {};
          ...
          if (payload.setDefault) {
            updatedData.budgets.categoryDefaults = 
              updatedData.budgets.categoryDefaults || {};
            updatedData.budgets.categoryDefaults[payload.category] = Number(payload.budget);
          } else {
            updatedData.budgets.categoryBudgets[payload.month][payload.category] = 
  ...
  const monthBudget = ledger.budgets?.months?.[selectedMonth] ?? ledger.budgets?.default ?? 0;
  ...
  function handleSaveCategory(category, isDefault = false) {
    ...
    if (isNaN(newBudget)) return;
    if (isDefault && !window.confirm(`Set default budget for ${category}?`)) return;
  ...
  return (
    <div className="p-6 space-y-8">
      ...
      {/* 保存成功提示 */}
      {showSaved && <div className="text-green-600 font-semibold">Saved!</div>}
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        ...
        <div className="p-4 rounded shadow bg-white text-center">
          ...
          <div className="text-xl font-bold">${(monthBudget - spent).toFixed(2)}</div>
        </div>
      </div>
      ...
      <div className="bg-white p-6 rounded shadow mb-8">
        ...
        <div className="flex items-center gap-2">
          ...
          />
          <button className="btn-primary" onClick={() => handleSaveMonth(false)}>Save</button>
          <button className="btn-ghost" onClick={() => handleSaveMonth(true)}>Set Default</button>
        </div>
      </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          {categories.map(cat => (
            <div key={cat.key} className="flex items-center gap-2">
              ...
              <input
                type="number"
                placeholder={catBudgets[cat.label] ?? defaultCatBudgets[cat.label] ?? ''}
                value={catEdits[cat.label]?.budget ?? ''}
                onChange={e => setCatEdits(prev => ({ ...prev, [cat.label]: { ...prev[cat.label], budget: e.target.value } }))}
                className="border p-2 rounded w-24"
              />
              <button className="btn-primary" onClick={() => handleSaveCategory(cat.label, false)}>Save</button>
              <button className="btn-ghost" onClick={() => handleSaveCategory(cat.label, true)}>Set Default</button>
            </div>
          ))}
        </div>
      </div>
